...
sub vcl_recv {
  ...
  } else if (req.url ~ "^/sampleapp/") {
	  # this is the sampleapp asking for a template. Point it to the default masters
    set req.backend_hint=appserver;		
    set req.url = regsub(req.url, "^/sampleapp/", "/master/");
	}
}

sub vcl_backend_response {
  # ensure backend response is not cached
  set beresp.ttl = 0s;
